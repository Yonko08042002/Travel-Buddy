...
>(
  (
    ...
    const handleRejectedFiles = useCallback(
      (rejectedFiles: FileRejection[], maxSize: number) => {
    ...
    const onDrop = useCallback(
      (acceptedFiles: File[], rejectedFiles: FileRejection[]) => {
        ...
        if (rejectedFiles.length > 0) {
          handleRejectedFiles(rejectedFiles, maxSize);
        }
      },
      [
        reSelectAll,
  ...
  return (
    ...
    >
      ...
      >
        ...
        <XIcon className='w-4 h-4 stroke-destructive duration-200 ease-in-out' />
      </button>
    </div>
  );
});
